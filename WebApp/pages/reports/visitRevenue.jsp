...
if(!page1.equals("")){
	...
	if(dateMap.size() > 0){
		...
		%><br/>
		<div id="visitRevenue-container" class="tab-container">
			<ul class='etabs'>
				<li class='tab' id="chartTab"><a href="#visitRevenueChart">Chart</a></li>
				<li class='tab' id="dataTab"><a href="#visitRevenueData">Data</a></li>
			</ul>
			
			<div id="visitRevenueChart">
				<center>
			...
			<div id="visitRevenueData">
				<table width="100%" id="visitRevenueCountTable" border="1" class="display" cellspacing="0" >
				<thead>
					<tr class="headerTR">
						<td>Date</td>
						<%
						for(String key : visitTypeMap.keySet()){
							%><td><%=visitTypeMap.get(key)%></td><%
						}
				...
				<tbody>
						...
						for(String key : dateMap.keySet()){
							...
							%>
								<tr align="center">
									<td><%=key %></td>
									<%
									for(String statusKey : visitTypeMap.keySet()){
					...
					for(String sKey : visitTypeMap.keySet()){
						chartString += "<dataset seriesname='" + visitTypeMap.get(sKey) + "' showvalues='0'>";
		...
		<script type="text/javascript">
		$('#visitRevenue-container').easytabs({
			uiTabs: true, 
			 collapsible: true ,
		...
		$(document).ready(function() {
			   var orderCounts = $('#visitRevenueCountTable').DataTable({
			    	"bSort" : true,
			    	"paging" : true/*,
			    	"pageLength": 15,
