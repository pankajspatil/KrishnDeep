...
public class Reports {
	...
	public LinkedHashMap<String, LinkedHashMap<String, String>> getVisitRevenueData(
			String fromDate, String toDate, String reportType,
			SessionModel sessionModel) throws SQLException {

		ConnectionsUtil connectionsUtil = new ConnectionsUtil();
		Connection conn = connectionsUtil.getConnection();

		String format = reportType.equalsIgnoreCase("month") ? "%b-%y"
				: "%d-%b-%y";
		String orderByFormat = reportType.equalsIgnoreCase("month") ? "%Y-%m"
				: "%Y-%m-%d";

		String query = "select v.visit_type_id, sum(ifnull(fees,0)) as amount, "
				+ "date_format(uv.created_on, '"
				+ format
				+ "') as reportKey "
				+ "from user_visit uv inner join visit_type v on uv.visit_type_id = v.visit_type_id and v.is_active = 1 "
				+ " inner join user_master u on u.user_id = uv.created_by and uv.created_by ="
				+ sessionModel.getSessionUserId()
				+ " "
				+ "and uv.created_on between ? AND ? group by reportKey "
				+ "order by date_format(uv.created_on, '"
				+ orderByFormat
				+ "') desc";
		...
		while (dataRS.next()) {
			...
			statusMap.put(dataRS.getString("visit_type_id"),
					dataRS.getString("amount"));
	...
	public LinkedHashMap<String, String> getVisitCount(String fromDate,
		...
		String query = "select uv.user_visit_id, count(*) as visitCount, "
				...
				+ "from user_visit uv "
				+ " inner join user_master u on u.user_id = uv.created_by and uv.created_by ="+ sessionModel.getSessionUserId()
				+ " where uv.created_on between ? AND ? group by reportKey "
				+ "order by date_format(uv.created_on, '" + orderByFormat
				+ "') desc , uv.user_visit_id;";
		...
		while (dataRS.next()) {
			...
			visitCountMap.put(dataRS.getString("reportKey"),
					dataRS.getString("visitCount"));
		}
	...
	public LinkedHashMap<String, String> getActiveVisitTypes()
			throws SQLException {
		...
		while (dataRS.next()) {
			visitTypeMap.put(dataRS.getString("visit_type_id"),
					dataRS.getString("visit_type"));
		}
